package com.example.stm32update.service;
...
import com.example.stm32update.utils.config.UUIDDataBase;
import com.example.stm32update.utils.tool.BWDataParser;
...
public class BluetoothService extends Service{
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            ...
            if(status==BluetoothGatt.GATT_SUCCESS){
                ...
                //Log.e(TAG,"discoverServiced is ok");
            }else {
                Log.e(TAG, "onServicesDiscovered received: " + status);   //发现设备的时候 发送广播
            }
        }
        ...
        public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                Log.e(TAG, "onCharacteristicRead被回调被触发,发送读信息广播");
            }
        }
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            ...
            broadcastUpdate(BroadCast.ACTION_DATA_AVAILABLE, characteristic);
           // Log.e(TAG,gatt+" "+characteristic+" "+characteristic.getValue().toString());
        }
    ...
    public void readCharacteristic(BluetoothGattCharacteristic characteristic) {
        ...
        //Log.e(TAG, "开始读蓝牙数据--中触发:onCharacteristicRead回调");
    ...
    public void setCharacteristicIndication(
        ...
        if (characteristic !=null){
            success= bluetoothGatt.setCharacteristicNotification(characteristic, enabled);
            if (success){
    ...
    另一个读设置
     */
    public boolean enableNotification(UUID serviceUUID, UUID characteristicUUID) {
        boolean success = false;
        BluetoothGatt gatt = bluetoothGatt;
        BluetoothGattService service = gatt.getService(serviceUUID);
        if (service != null) {
            Log.e(TAG,"xiaowenlg:service is not null");
            BluetoothGattCharacteristic characteristic = findNotifyCharacteristic(service, characteristicUUID);
            Log.e(TAG,"Mcharacterstic is:"+characteristic.getUuid().toString());
            if (characteristic != null) {
    ...
    private BluetoothGattCharacteristic findNotifyCharacteristic(BluetoothGattService service, UUID characteristicUUID) {
        ...
        for (BluetoothGattCharacteristic c : characteristics) {
            if ((c.getProperties() & BluetoothGattCharacteristic.PROPERTY_NOTIFY) != 0
                ...
                characteristic = c;
                Log.e(TAG,"xiaowenlg:findNotifyCharacteristic:"+characteristic.getUuid().toString());
                break;
            }
        }
        if (characteristic != null)
    ...
    public boolean SendPassWord(BluetoothGattCharacteristic charac,byte[] message){
        //check mBluetoothGatt is available
        if (bluetoothGatt == null) {
            Log.e(TAG, "lost connection");
            return false;
    ...
    public boolean writeBle(BluetoothGattCharacteristic charac,byte[] message){
        //check mBluetoothGatt is available
        if (bluetoothGatt == null) {
            Log.e(TAG, "lost connection");
            return false;
